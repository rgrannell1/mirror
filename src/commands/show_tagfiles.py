...
def show_tagfiles(dir: str, tag: str = None):
  ...
  for tagfile in PhotoVault(dir, metadata_path=None).list_tagfiles():
    ...
    if any(image for image in tag_file['images'].values() if tag in image.get(ATTR_TAG, [])):
      images = tag_file['images'].values()

      tag_file['metadata'] = {
        'fpath': tagfile,
        'total_images': len(images),
        'published': len([image for image in images if 'Published' in image.get(ATTR_TAG, [])]),
        'untagged_published': len([image for image in images if len(image.get(ATTR_TAG, [])) == 1]),
      }
