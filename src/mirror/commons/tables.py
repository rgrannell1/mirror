...
cover_photos AS (
    ...
    cover_with_thumbnail AS (
        SELECT
            p.dpath,
            MAX(p.fpath) AS cover
        FROM photos p
        JOIN encoded_photos ep
            ON ep.fpath = p.fpath
            AND ep.role = 'thumbnail_lossy'
        WHERE p.fpath LIKE '%+cover%'
        GROUP BY p.dpath
    )
    SELECT
        ca.dpath,
        COALESCE(cwt.cover, ca.cover) AS cover
    FROM cover_any ca
    LEFT JOIN cover_with_thumbnail cwt
        ON ca.dpath = cwt.dpath
)
SELECT
    (
