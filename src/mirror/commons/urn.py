...
def split_urn_query(remainder: str) -> tuple[str, dict]:
    """Split 'noun:id?query' into the main part and a parsed query dict."""
    if "?" in remainder:
        main_part, query_part = remainder.split("?", 1)
        return main_part, dict(urllib.parse.parse_qsl(query_part))
...
def parse_mirror_urn(urn: str) -> dict:
    ...
    main_part, qs = split_urn_query(urn[len(URN_PREFIX) :])
