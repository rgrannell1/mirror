...
class Tagfile:
    ...
    def album_data(self) -> List[Dict[str, Any]]:
        ...
        return [
            {
                ATTR_ALBUM_TITLE: album_md.title
                if album_md and album_md.title
                else self.dirname,
                ATTR_ALBUM_COVER: album_md.cover
                if album_md and album_md.cover
                else "Cover",
                ATTR_ALBUM_DESCRIPTION: album_md.description
                if album_md and album_md.description
                else "",
                ATTR_ALBUM_ID: self.id(),
                ATTR_ALBUM_GEOLOCATION: album_md.geolocation
                if album_md and album_md.geolocation
                else "",
                ATTR_ALBUM_PERMALINK: album_md.permalink
                if album_md and album_md.permalink
                else "",
                "images": images,
                "videos": videos,
            }
    ...
    def read(kls, fpath: str) -> Optional[Dict]:
        ...
        if (
            f"![{cover}]({cover})" not in tag_file["images"]
            and cover != "Cover"
            and cover != "Unknown"
        ):
